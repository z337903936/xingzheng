  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="状态" align="center" width="120px">
        <template v-if="row.evidenceMaterialResult" slot-scope="{row}">
      ...
      <el-table-column label="操作" align="center" fixed="right" width="280" class="small-padding fixed-width">
        <template slot-scope="{row}">
          ...
          <!--</router-link>-->
          <el-button v-waves :disabled="resultDisable(row)" type="success" size="mini" style="width: 90px" icon="el-icon-tickets" @click="gotoMaterialResult(row)">填写结果</el-button>
    ...
    <el-dialog :close-on-click-modal="false" :visible.sync="dialogTransFrom" title="移交鉴定文书" width="50%">
      <el-form
        ref="trans"
        :model="transFrom"
        label-position="left"
    ...
    <el-dialog :close-on-click-modal="false" :visible.sync="dialogTaskFrom" title="填写任务号" width="50%">
      <el-form
        ref="trans"
        :model="taskFrom"
        label-position="left"
        label-width="80px"
        style="width: 80%; margin-left:50px;">
        <el-form-item v-if="batch.stepName === 'DNA送检'" label="任务号" prop="dnaTaskNo">
          <el-input v-model="taskFrom.dnaTaskNo"/>
        </el-form-item>
        <el-form-item v-if="batch.stepName === '指纹送检'" label="任务号" prop="fpTaskNo">
          <el-input v-model="taskFrom.fpTaskNo"/>
        </el-form-item>
        <el-form-item v-if="batch.stepName === '电子物证送检'" label="任务号" prop="digiTaskNo">
          <el-input v-model="taskFrom.digiTaskNo"/>
        </el-form-item>
        <el-form-item v-if="batch.stepName === '理化送检'" label="任务号" prop="cheTaskNo">
...
export default {
  ...
  methods: {
    ...
    resultDisable(row) {
      ...
      if (this.$store.getters.id !== row.evidenceMaterialResult.createUid) { return true }
    ...
    writeResult() {
      ...
      if (data.checkOutTime && data.checkOutTime.toString().length > 10) { data.checkOutTime = parseInt(data.checkOutTime / 1000) }
      if (data.checkOutTime === '') { data.checkOutTime = 0 }
