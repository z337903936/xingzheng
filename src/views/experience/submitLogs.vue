...
<script>
  import { fetchDealerSubmitLogs, setDealerSubmitLogStatus } from '@/api/dealerExperience'
  ...
  export default {
    ...
    methods: {
      ...
      handleChangeStatus() {
        ...
        setDealerSubmitLogStatus(data).then(data => {
          this.loading = false
          if (data.code == 200) {
            this.$message({
              message: '保存成功',
      ...
      getList() {
        ...
        fetchDealerSubmitLogs(param).then(data => {
          this.list = data.list
