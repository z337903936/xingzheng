    ...
    >
      <el-table-column ref="expand" type="expand">
        <template slot-scope="{row}">
          <el-form v-if="row.status === -2" label-position="left" inline class="table-expand">
      ...
      <el-table-column label="简要案情" align="center" style="min-width: 600px">
        <template v-if="row.evidence" slot-scope="{row}">
      ...
      <el-table-column label="操作" align="center" width="230" class-name="small-padding fixed-width">
        ...
        <template slot-scope="{row}">
          ...
          <template v-if="row.evidence">
            <router-link v-if="row.stepName === '痕检现勘' || row.stepName === '警情扭转'" :to="'/search/update-search/'+row.evidence.id">
