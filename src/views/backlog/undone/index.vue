<template>
  <div class="app-container">
    ...
    >
      <el-table-column ref="expand" type="expand">
        <template slot-scope="{row}">
          <el-form v-if="row.fromStep === '接警'" label-position="left" inline class="table-expand">
          ...
          <el-form v-else-if="row.fromStep === '痕检现勘'" label-position="left" inline class="table-expand">
      ...
      <el-table-column label="操作" align="left" width="430" fixed="right" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button
            v-if="row.status===1 && (row.stepName === '申请物证入库' || row.stepName === '申请物证出库')"
            type="primary"
            size="small"
            icon="el-icon-document-checked"
            @click="handleAction(row,true)">
            同&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;意
          </el-button>
          <el-button
            v-if="row.status===1 && (row.stepName === '申请物证入库' || row.stepName === '申请物证出库')"
          ...
          <el-button
            ...
            @click="gotobatchList(row)">
            物证详情</el-button>
          <router-link
            v-if="row.status===2 && (row.stepName==='痕检现勘' || row.stepName === '警情扭转' ) "
            :to="'/search/update-search/'+row.evidenceId">
          ...
          <el-button
            ...
            size="small"
            icon="el-icon-tickets"
            @click="handleCancelEvidence(row.record)">
            取消勘查
          </el-button>
          <router-link v-if="row.stepName === '痕检现勘'" :to="'/search/show-search/'+row.evidence.id">
    ...
    <!--&gt;-->
    <!--</el-pagination>-->
    <el-dialog :close-on-click-modal="false" :visible.sync="dialogFormAction" title="任务" width="50%">
      <el-form
        ref="acceptTaskFrom"
    ...
    <el-dialog :close-on-click-modal="false" :visible.sync="dialogFormAccept" title="接受任务" width="50%">
      <el-form
        ref="acceptTaskFrom"
        :model="acceptTaskFrom"
        label-position="left"
    ...
    <el-dialog :close-on-click-modal="false" :visible.sync="dialogresultFrom" title="填写结果" width="50%">
      <el-form
        ref="resultFrom"
        :model="resultFrom"
        label-position="left"
        ...
        <el-form-item label="是否推送给主办痕检" prop="needToPushToCharger">
          <el-checkbox v-model="resultFrom.needToPushToCharger"/>
        </el-form-item>
        ...
        <el-form-item label="委托人" prop="delegateName">
          <el-select
            v-model="resultFrom.delegateUid"
...
export default {
  ...
  methods: {
    ...
    writeResult() {
      const data = Object.assign({}, this.resultFrom)
      if (data.documentDate.toString().length > 10) { data.documentDate = parseInt(data.documentDate / 1000) }
    ...
    handleAcceptTaskSeach(task) {
      const data = this.acceptTaskFrom
      data.stepId = task.id
    ...
    actionTask(type) {
      const data = this.acceptTaskFrom
      data.agree = type
      if (data.requireTime.toString().length > 10) { data.requireTime = parseInt(data.requireTime / 1000) }
      accetpTask(data).then(response => {
        if (response.code === 200) {
          this.$message({
            message: '操作成功',
