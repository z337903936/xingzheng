  ...
  methods: {
    ...
    fetchData(id) {
      fetchMaterial(id).then(data => {
        ...
        if (this.postForm.transferUid === 0) { this.postForm.transferUid = '' }

        if (this.postForm.transferTime.toString().length === 10) { this.postForm.transferTime = data.transferTime * 1000 } else if (this.postForm.transferTime === 0) { this.postForm.transferTime = '' }
      }).catch(err => {
        console.log(err)
      })
    },
    submitForm() {
      ...
      if (data.transferTime.toString().length > 10) { data.transferTime = parseInt(data.transferTime / 1000) }
