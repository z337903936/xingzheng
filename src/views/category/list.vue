<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column min-width="200px" label="顶级分类">
        <template slot-scope="scope">
          <span>{{ scope.row.categoryType == 1?"新闻":"业务" }}</span>

        </template>
      </el-table-column>

      <el-table-column width="300px" label="分类介绍图">
        <template slot-scope="scope">
          <img :src="scope.row.headPic" class="head_img">
        </template>
      </el-table-column>
...
<script>
import { fetchCateList, delCategory } from '@/api/category'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchCateList(this.listQuery).then(data => {
        this.list = data.list
